...
| 20 | Auto-fix Task | done — mutating `fix` lifecycle task in the toolchain abstraction (python-default `ruff check --fix .`; JSON stacks declare their own), listed in `MUTATING_TASKS`; `run_toolchain_task` runs it only under the `run_toolchain_task_fix` capability — enabled for the implementer, refused (ToolError) for the read-only Verifier so the gate stays honest; SystemArchitect persona acts on tool auto-fix recommendations (tests: `test_toolchain_fix.py`) | [ADR-019](adr/ADR-019-toolchain-fix-task.md) |
| 19 | Monitoring Dashboard | done — `bin/audit_log.py` appends structured JSONL events (one `O_APPEND` write/line ⇒ readers see at worst a torn final line); `harness run` emits `session_start`/`session_end`; `bin/ui_dashboard.py` is a read-only Streamlit Observer — newest-first `selectbox`, chronological timeline, colored toolchain/gate status, payload/result in expanders, robust parser + refresh/auto-refresh. Real events wired: `agent_turn` (per attempt + verifier, both backends), `execute_toolchain_task` (`run_toolchain_task` tool + gate), `mechanical_gate` verdict (each with a `duration_ms`; agent turns are paired start/`agent_turn_done`) — via `mcp_server/core/audit.py` `emit_env` (env-driven, best-effort) with `HARNESS_AUDIT_DIR`/`HARNESS_ACTOR` propagated down the process tree (tests: `test_ui_dashboard.py`, `test_audit_instrumentation.py`) | [ADR-018](adr/ADR-018-monitoring-dashboard.md) |
| 18 | Unified CLI Entry Point | done — root `./harness` (bash, `set -euo pipefail`): `install` (venv + deps incl. streamlit, verifies MCP entry points, prepares gitignored `.agent-harness/audit_logs/`), `run "<task>" [args…]` (unique exported `SESSION_ID`, tees to the audit log, forwards to `run_agent.sh`, preserves its exit code), `ui` (read-only Streamlit Observer `bin/ui_dashboard.py` over the audit logs) (tests: `test_harness_cli.py`) | [ADR-017](adr/ADR-017-unified-cli-entrypoint.md) |
| 17 | Project Initialization | done — `bin/init_project.sh` scaffolds a target (constitution/CLAUDE.md/specs/toolchain.json/git baseline) with a chosen stack + agent backend + directory (tests: `test_init_project.py`) | [ADR-016](adr/ADR-016-project-initialization.md) |
| 16 | Toolchain Abstraction | done — `core/toolchain.py` (`resolve_toolchain` → JsonToolchain\|PythonToolchain, sanitize + head/tail truncate), capability-gated `run_toolchain_task` tool, `completion_checks.py` routes lint via the abstraction; `toolchain.json` overrides per stack, absent = Python default (tests: `test_toolchain.py`) | [ADR-015](adr/ADR-015-toolchain-abstraction.md) |
...
| `mark_spec_complete` | — | touches `.agent-harness/spec_complete`; registered ONLY when the server is started `--enable-tool mark_spec_complete` (Verifier's server); the loop's success condition (ADR-012) |
| `run_toolchain_task` | `task: install\|lint\|test\|build` | capability-gated (`--enable-tool run_toolchain_task`; implementer + Verifier); runs the task via `<target>/toolchain.json` or the Python default; JSON `{task, stack, command, status, exit_code, output_snippet}` — snippet ANSI-stripped + head(20)/tail(50) truncated with rescued error lines, ≤ 4 KiB (ADR-015) | `mcp_server/tools/git_tools.py` |
