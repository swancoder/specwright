...
- `main.py` — `build_registry`, `build_server` (`mcp.server.Server` with `on_list_tools`/`on_call_tool`), `serve` over `stdio_server()`, CLI `--target-dir` (required) `--db` `--write-scope <prefix>` (repeatable; ADR-011).
- `core/context.py` — `HarnessContext.write_scopes` / `allows_write()`; `fs_write`/`fs_apply_patch` raise `SandboxViolation` outside every scope (reads are never scoped).
...
| `git_commit_feature` | `message, spec_id: str` | validates Conventional Commits, header `… [spec_id]`, stages `-A` excluding `CLAUDE.md`, `prompts-hist/`, `.agent-harness/`, `.venv/`, `__pycache__/`, `*.db`, `*.sqlite`, `*.sqlite3`, `*.db3` (ADR-010); no co-author trailers; on success touches `.agent-harness/run_successful` (ADR-008) | `mcp_server/tools/git_tools.py` |
...
- The `run_successful` marker means *a* commit happened, not that the spec is complete; a legitimate agent stop (reported conflict or ADR-010 stuck rule) is still retried up to `MAX_RETRIES` (ADR-008).
...
## 7. Roadmap
- `pyproject.toml`/`pip install -e .` provisioning; `blocked` marker to short-circuit retries on legitimate stops; branch creation (`feat/<NNN>-<slug>`) as a harness step; milestone-aware loop; native Ollama adapter in `opencode.json` to separate gpt-oss reasoning; first end-to-end spec run and transcript review; OS-level sandbox for `run_tests`; rename tracking in the indexer.
