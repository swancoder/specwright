...
- `llm_backends.yaml` — `default: <name>` + `backends.<name>: {provider, base_url, model, api_key, api_key_env?, extra_env?}`. Profiles: `ollama` (`http://localhost:11434/v1`, `qwen2.5-coder:14b-32k`, placeholder key), `freetoken`. Exported as `OPENAI_BASE_URL`, `OPENAI_API_KEY`, `MODEL_NAME`, `LLM_BACKEND`, `LLM_PROVIDER` (no Anthropic-specific variables — ADR-006); `api_key_env` overrides `api_key` when set in the caller's env.
