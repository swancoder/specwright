...
| `mcp_server/` | The MCP boundary the agent acts through: a strict tool registry (`fs_read/write/list`, `apply_patch`, `run_tests`, `git_commit_feature`, `run_toolchain_task` (incl. a capability-gated `fix` task the implementer runs to apply auto-fixes like `ruff --fix`), `query_temporal_coupling`, …) with per-role gating. |
